...
namespace runnerDotNet
{
    public class RunnerDBReader : XClass
    {
        ...
        public RunnerDBReader(DbDataReader reader)
        {
			...
			if(dbReader is Npgsql.NpgsqlDataReader)
				ConnectionType = Constants.nDATABASE_PostgreSQL;
        }
            ...
            if (buffer != null)
            {
                if (!IsLastRecordReaded || !dbReader.IsClosed)
                {
                    ...
                    for (int i = 0; i < dbReader.FieldCount; i++)
                    {
						...
						if(ConnectionType == Constants.nDATABASE_PostgreSQL)
						{
							if (value is DateTime && dbReader.GetProviderSpecificFieldType(i) == typeof(NpgsqlTypes.NpgsqlTime))
							{
